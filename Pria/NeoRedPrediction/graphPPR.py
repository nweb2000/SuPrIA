...
def printGraphRecommendedDocs(G,list_nodes, R):
    ...
    result = False
    jsonData['GoogleNews'][Constants.GRAPH]=[]
    recommInfo = {}
    graphDocs = {}
    ...
    try:
        ...
        for (key,val) in sorted_x:
            if key in recommInfo:
                ...
                linkObj['rank'] = -1
                jsonData['GoogleNews'][Constants.GRAPH].append(linkObj)
                count = count + 1
                if count >= graphConstants.RECOMMENDED_LINKS:
                    break
            else:
                 graphUtils.logger.error("Graph Google key not found = "+key)
    ...
    if jsonData is None:
        return False    
    jsonData['suggestGoogle'][Constants.GRAPH]=[]
    recommInfo = {}
    graphDocs = {}
    ...
    try:
        ...
        for (key,val) in sorted_x:
            if key in recommInfo:
                linkObj = recommInfo[key]
                linkObj['rank'] = -1
                jsonData['suggestGoogle'][Constants.GRAPH].append(linkObj)
                count = count + 1
                if count >= graphConstants.RECOMMENDED_LINKS:
                    break
            else:
                graphUtils.logger.error("Graph suggest Google key not found = "+key)
        json.dump(jsonData, outfile)
        outfile.close()
        result = True
        graphUtils.logger.info("Simple graph recommendation done for today")
    except Exception, e:
        graphUtils.logger.error("Exception = %s" % e)
        graphUtils.logger.error("Exception at writing final Graph Recommendation docs for data : %s" % write_directory)
    return result
...
def PPR():
    ...
    lastRecommendationnDate = graphUtils.loadSettings(graphConstants.LAST_GRAPH_RECOMM_DONE)
    #lastRecommendationnDate = None
    if todayDate == lastRecommendationnDate:
        graphUtils.logger.info("Simple Graph recommendation PPR done for today ")
        return
    graphUtils.logger.info("Simple graph recommendation PPR last done for ="+str(lastRecommendationnDate))
    ...
    for idx,node in enumerate(list_seednode_names):
        graphUtils.logger.info(str(idx)+" seed node for simple graph today = "+node)

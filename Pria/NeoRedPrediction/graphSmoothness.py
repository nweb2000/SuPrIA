...
def Smoothness():
    ...
    lastSmoothnessDate = graphUtils.loadSettings(graphConstants.LAST_GRAPH_SMOOTHNESS_DIR)
    lastSuggSmoothnessDate = graphUtils.loadSettings(graphConstants.LAST_GRAPH_SUGG_SMOOTHNESS_DIR)
    
    if lastSmoothnessDate:
        graphUtils.logger.info("Graph Smoothness done last for ="+lastSmoothnessDate)
    else:
        graphUtils.logger.info("Graph Smoothness done last for None")
        
    if lastSuggSmoothnessDate:
        graphUtils.logger.info("GraphSugg Smoothness done last for ="+lastSuggSmoothnessDate)
    else:
    ...
    if todayDate == lastSmoothnessDate and todayDate == lastSuggSmoothnessDate:
        graphUtils.logger.info("Graph Smoothness signal already done for today :" + todayDate)
        return True
    ...
    if todayDate != lastSmoothnessDate:    
        ...
        graphUtils.logger.info("Simple graph Smoothness completed for normalGoogle today. Stats follow")
        ...
        graphUtils.saveSettings(graphConstants.LAST_GRAPH_SMOOTHNESS_DIR, todayDate)
        pass   
    ...
    if todayDate != lastSuggSmoothnessDate:    
        ...
        for testDoc in range(len(testCorpus)):
                ...
                #print smoothness[testDoc]
        graphUtils.logger.info("Simple graph Smoothness completed for suggGoogle today. Stats follow")
        graphUtils.logger.info("mini =" + str(mini))
        graphUtils.logger.info( "maxi =" + str(maxi))
        graphUtils.logger.info( "Smoothness edges count ="+ str(count))
        nx.write_gexf(G, write_graph_file)
        graphUtils.saveSettings(graphConstants.LAST_GRAPH_SUGG_SMOOTHNESS_DIR, todayDate)
        pass   

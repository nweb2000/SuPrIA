...
def Relevance():
    ...
    lastRelevanceDate = graphUtils.loadSettings(graphConstants.LAST_GRAPH_RELEVANCE_DIR)
    lastSuggRelevanceDate = graphUtils.loadSettings(graphConstants.LAST_GRAPH_SUGG_RELEVANCE_DIR)
    
    if lastRelevanceDate:
        graphUtils.logger.info("Graph Relevance done last for ="+lastRelevanceDate)
    else:
        graphUtils.logger.info("Graph Relevance done last for None")
        
    if lastSuggRelevanceDate:
        graphUtils.logger.info("GraphSugg Relevance done last for ="+lastSuggRelevanceDate)
    else:
    ...
    if todayDate == lastRelevanceDate and todayDate == lastSuggRelevanceDate:
        graphUtils.logger.info("Graph Relevance signal already done for today :" + todayDate)
        return True
    ...
    if todayDate != lastRelevanceDate:
        ...
        graphUtils.logger.info("Simple graph relevance completed for today. Stats follow")
        ...
        graphUtils.saveSettings(graphConstants.LAST_GRAPH_RELEVANCE_DIR, todayDate)
    ...
    if todayDate != lastRelevanceDate:
        ...
        for idx,text in enumerate(testCorpus):
                ...
                text = readFromFile(testFiles[idx])
                #NERFunc(text,G, recomm_nodename)
        graphUtils.logger.info("Simple graph relevance completed for suggestGoogle today. Stats follow")
        graphUtils.logger.info("mini =" + str(mini))
        graphUtils.logger.info( "maxi =" + str(maxi))
        graphUtils.logger.info( "Relevance count ="+ str(count))
        nx.write_gexf(G, write_graph_file)
        graphUtils.saveSettings(graphConstants.LAST_GRAPH_SUGG_RELEVANCE_DIR, todayDate)
    pass

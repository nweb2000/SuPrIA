...
def findTrainingDays():
    ...
    for i in range(Constants.MAX_PREVIOUS_DAYS):
...
def findTrainingFiles():
    ...
    for day in dates:
        if completed_days == Constants.TRAINING_DAY:
            break
        folders = []
        day_found = False
        folders.append(os.path.join(Constants.ROOT_FOLDER,Constants.BOILER_DATA_DIR,day))
        folders.append(os.path.join(Constants.ROOT_FOLDER,Constants.DATA_DIR,day,Constants.FACEBOOK_DIR))
        folders.append(os.path.join(Constants.ROOT_FOLDER,Constants.DATA_DIR,day, Constants.TWITTER_DIR))
...
def findTestFiles():
    ...
    folders = os.path.join(Constants.ROOT_FOLDER,Constants.RECOMMENDATION_DIR,Constants.BOILER_GOOGLE_NEWS_DIR,day, Constants.GOOGLENEWS)
...
def findSuggTestFiles():
    onlyfiles = []
    day = getTodayDateFolder()
    folders = os.path.join(Constants.ROOT_FOLDER,Constants.RECOMMENDATION_DIR,Constants.BOILER_GOOGLE_NEWS_DIR,day, Constants.SUGG_GOOGLENEWS)
    if os.path.exists(folders):
        onlyfiles.extend([os.path.join(folders,
                     fi) for fi in os.listdir(folders) if os.path.isfile(os.path.join(folders,
                     fi))])
...
def random_select(Corpus):
     ...
     return Corpus[:Constants.MAX_FILES_SMOOTHNESS]

...
def readLinksJson(downloadDate):
    ...
    try:
        ...
        util.logger.error("Exception at read Boiler Google recomm json for : %s" % downloadDate)
    return jsonData
...
def printRecommendedDocs(recDocs, downloadDate):
    ...
    result = False
    jsonData['GoogleNews'][Constants.BASELINE]=[]
    recommInfo = {}
    ...
    try:
        ...
        for (key,val) in sorted_x:
            if key in recommInfo:
                ...
                linkObj['rank'] = -1
                jsonData['GoogleNews'][Constants.BASELINE].append(linkObj)
                count = count + 1
                if count >= Constants.RECOMMENDED_LINKS:
                    break
            else:
                util.logger.error("Key not found in baseline google news = "+key)
        ...
        util.logger.error("Exception at writing final Google Recommendation docs for data : %s" % write_directory)
    return result

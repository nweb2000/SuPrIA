...
def ConnectionClarity():
    ...
    lastClarityDate = graphUtils.loadSettings(graphConstants.LAST_GRAPHNER_CLARITY_DIR)
    lastSuggClarityDate = graphUtils.loadSettings(graphConstants.LAST_GRAPHNER_SUGG_CLARITY_DIR)
    if lastClarityDate:
        graphUtils.logger.info("NERGraph Google Clarity done last for ="+lastClarityDate)
    else:
        graphUtils.logger.info("NERGraph Google Clarity done last for none")
        
    if lastSuggClarityDate:
        graphUtils.logger.info("NERGraph Sugg Clarity done last for ="+lastSuggClarityDate)
    else:
        graphUtils.logger.info("NERGraph Sugg Clarity done last for none")
        
    if todayDate == lastClarityDate and todayDate == lastSuggClarityDate:
        graphUtils.logger.info("NERgraph Clarity signal done for today =" + todayDate)
        return True
    ...
    if todayDate != lastClarityDate:
        ...
        graphUtils.logger.info("Simple  ner graph clarity completed for normalGoogle today. Stats follow")
        ...
        graphUtils.saveSettings(graphConstants.LAST_GRAPHNER_CLARITY_DIR, todayDate)
        pass 
    ...
    if todayDate != lastSuggClarityDate:
        ...
        graphUtils.logger.info("Simple  ner graph clarity completed for suggGoogle today. Stats follow")
        graphUtils.logger.info("mini =" + str(mini))
        graphUtils.logger.info( "maxi =" + str(maxi))
        graphUtils.logger.info( "clarity edges count ="+ str(count))
        nx.write_gexf(G, write_graph_file)
        graphUtils.saveSettings(graphConstants.LAST_GRAPHNER_SUGG_CLARITY_DIR, todayDate)
        pass 
class Clarity:
